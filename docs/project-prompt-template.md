...
5. Treat chat-supplied files and context as the primary domain source, and the bridge as the conversation history between models. If the overview lists relevant session or group files, read text/PDF files with `download_session_file(session_id=..., file_id=...)` and images with `view_session_image(session_id=..., file_id=...)` before answering.
...
2. Use `list_session_files(session_id=...)` to inspect files visible to the current session and `download_session_file(session_id=..., file_id=...)` to read text/PDF content, or `view_session_image(session_id=..., file_id=...)` to view an image. Always pass the same known `session_id`; never supply or infer a separate group for file reads.
...
5. For a relevant file with `content_kind: image`, call `view_session_image(session_id=..., file_id=...)` instead of the text download tool. To save an accessible JPEG/PNG, use `upload_session_image` or `upload_group_image` with base64 generated by code from the original bytes (10 MB, 40 MP, static images only). Do not reconstruct base64 from visual perception. If your client does not support native image tool results or cannot access the source file, say so; do not claim to have viewed or uploaded it.
6. Uploaded files are local bridge runtime data — do not imply they are committed to the public repository.
